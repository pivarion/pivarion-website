...
const bytes = fs.readFileSync(path.join(root, 'assets/models/porsche-911-gt3-rs.glb'));
...
const alignment = JSON.parse(fs.readFileSync(path.join(root, 'assets/models/normalization.json')));
new THREE.GLTFLoader().parse(patched.buffer.slice(patched.byteOffset, patched.byteOffset+patched.byteLength),'', gltf => {
  const result = PivarionVehicle.adapt(gltf,alignment);
  ...
  const rimMaterial=result.wheels[0].userData.turn.children.find(x=>x.material.name==='Paint1Mtl').material;
  assert.equal(rimMaterial.color.clone().convertLinearToSRGB().getHex(),0xb81910,'rim swatch colour-space mismatch');
  assert.equal(rimMaterial.roughnessMap,null,'source mask overrides rim coating roughness');
  for (const wheel of result.wheels) {
    assert(wheel.userData.turn.children.some(x=>x.material.name==='Paint1Mtl'),'red rim missing');
    assert(wheel.userData.turn.children.some(x=>x.material.name==='Tire1Mtl'),'tire missing');
    assert(wheel.children.some(x=>x.isMesh && x.material.name==='Caliperlf0021Mtl'),'stationary caliper missing');
  }
  ...
  assert(size.y>1.2 && size.y<1.5,'unexpected car height');
  assert(size.z>1.8 && size.z<2.2,'unexpected car width');
  assert(Math.abs(box.min.y)<.001,'tires not on ground');
  const caliper = result.wheels[0].children.find(x=>x.isMesh && x.material.name==='Caliperlf0021Mtl');
  ...
  console.log(JSON.stringify({meshes,triangles,bounds:box,size,stationaryCalipers:true,fourRotatingWheels:true},null,2));
